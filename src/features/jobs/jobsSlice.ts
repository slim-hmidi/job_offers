...
export const jobsAdapter = createEntityAdapter({
  sortComparer: (a: FormattedJobResponse, b: FormattedJobResponse) =>
...
const jobSlice = createSlice({
  ...
  extraReducers: {
    [`${fetchJobs.pending}`]: (
      state,
      action: PayloadAction<FormattedJobResponse[]>,
    ) => {
    ...
    [`${fetchJobs.rejected}`]: (
      state,
      action: PayloadAction<string, string, unknown, Error>,
    ) => {
...
export const jobFilterSelector = (state: RootState): Filter =>
  state.jobs.filters
  ...
  (jobs, filterOptions) =>
    jobs
      ...
      .filter(
        job =>
          !filterOptions.title ||
          job.title.toLowerCase().includes(filterOptions.title.toLowerCase()),
      )
      .filter(
        job =>
          !filterOptions.fullTime ||
          job.type.toLocaleLowerCase() === 'full time',
      )
      .map(job => job.id),
)
